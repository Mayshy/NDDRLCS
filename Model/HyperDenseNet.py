...
from Model._utils import testModel, testBackward, test2IModel, test2IBackward
...
def convBlock(nin, nout, kernel_size=3, batchNorm=False, layer=nn.Conv2d, bias=True, dropout_rate=0.0, dilation=1):
    if batchNorm == False:
        return nn.Sequential(
            ...
            layer(nin, nout, kernel_size=kernel_size, bias=bias, padding=1, dilation=dilation)
        )
    else:
        return nn.Sequential(
            nn.BatchNorm2d(nin),
...
class HyperDenseNet_2Mod(nn.Module):
    def __init__(self, nClasses, mod0_channels=3, mod1_channels=3):
        ...
        # Path-Top
        self.conv1_Top = convBlock(mod0_channels, 25)
        ...
        # Path-Bottom
        self.conv1_Bottom = convBlock(mod1_channels, 25)
    ...
    def forward(self, x, y):
        # ----- First layer ------ #
        # get 2 of the channels as 4D tensors
        # pdb.set_trace()
        # y1t = self.conv1_Top(input[:, 0:1, :, :])
        # y1b = self.conv1_Bottom(input[:, 1:2, :, :])
        y1t = self.conv1_Top(x)
        y1b = self.conv1_Bottom(y)
...
class HyperDenseNet(nn.Module):
    def __init__(self, nClasses):
        ...
        self.fully_1 = nn.Conv2d(4050, 400, kernel_size=1)
        self.fully_2 = nn.Conv2d(400, 200, kernel_size=1)
        self.fully_3 = nn.Conv2d(200, 150, kernel_size=1)
        self.final = nn.Conv2d(150, nClasses, kernel_size=1)
    ...
    def forward(self, input):
        # ----- First layer ------ #
        # get the 3 channels as 4D tensors
        y1t = self.conv1_Top(input[:, 0:1, :, :])
