...
def sum_truncated_loss_grad(theta, X, Y, idx1, idx0, _lambda):
    ...
    if _lambda == 0:
        ...
        if _lambda > 0:
            ...
            loss = (np.log(_lambda / p1)) * np.sum(cond_g1, 0) + np.sum(
                _dot_intercept(theta, X[cond_g1]) * (1 - Y[cond_g1]), 0) \
                   + (np.log(_lambda / p0)) * np.sum(cond_g0, 0) + np.sum(
                -_dot_intercept(theta, X[cond_g0]) * Y[cond_g0], 0)
        else:
            ...
            loss = (np.log(-_lambda / p1)) * np.sum(cond_g1, 0) + np.sum(
                -_dot_intercept(theta, X[cond_g1]) * Y[cond_g1], 0) \
                   + (np.log(-_lambda / p0)) * np.sum(cond_g0, 0) + np.sum(
                _dot_intercept(theta, X[cond_g0]) * (1 - Y[cond_g0]), 0)
...
class fair_logloss_classifier:
    ...
    def fit(self, X, Y, A):
        ...
        res = minimize(self.compute_loss_grad, theta, args=(X, Y), method='L-BFGS-B', jac=True, tol=self.tol,
    ...
    def _predict_proba_given_y(self, X, Y, _lambda, grp1, grp2, p1, p2):
        ...
        if _lambda > 0:
            ...
            pcheck[grp1] = np.where(cond[grp1], np.ones_like(pcheck[grp1]),
                                    phat[grp1] * (1 + (_lambda / p1) * (1 - phat[grp1])))

            cond = (1 - (p2 / _lambda)) > phat
            phat[grp2] = np.maximum(phat[grp2], 1 - p2 / _lambda)
            pcheck[grp2] = np.where(cond[grp2], np.zeros_like(cond[grp2]),
                                    phat[grp2] * (1 - (_lambda / p2) * (1 - phat[grp2])))
        elif _lambda < 0:
            cond = (1 + (p1 / _lambda)) > phat
            phat[grp1] = np.maximum(phat[grp1], 1 + p1 / _lambda)
            pcheck[grp1] = np.where(cond[grp1], np.zeros_like(cond[grp1]),
                                    phat[grp1] * (1 - (_lambda / p1) * (1 - phat[grp1])))

            cond = (- p2 / _lambda) < phat
            phat[grp2] = np.minimum(phat[grp2], - p2 / _lambda)
            pcheck[grp2] = np.where(cond[grp2], np.ones_like(cond[grp2]),
                                    phat[grp2] * (1 + (_lambda / p2) * (1 - phat[grp2])))
        return phat, pcheck
...
# DP_fair_logloss_classifier, EOPP_fair_logloss_classifier, EODD_fair_logloss_classifier
if __name__ == '__main__':
    ...
    loss = DP_fair_logloss_classifier()
    testLoss(model, loss)